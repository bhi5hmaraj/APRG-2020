...
for i in range(0,NumberOfOperations):
    ...
    if inp[0] == "+":
        ...
        result = RBObject.findCount(RBObject.root, int(inp[1]), int(inp[2]))
        # result = len(list(x for x in entered if int(inp[1]) <= x <= int(inp[2])))
        res.append(str(result))

print("\n".join(res))

class BlackRedNode(object):
...
class BlackRedTree(object):
    ...
    def findCount(self, root, low, high): 
        ...
        if root.val <= high and root.val >= low:  
            return (1 + self.findCount(root.left, low, high) + self.findCount(root.right, low, high))
                         

        elif root.val < low:  
            return self.findCount(root.right, low, high) 
  
        else: 
            return self.findCount(root.left, low, high)
        
        def insertTo(self, root, val):
        if not root:
            return BlackRedNode(val)
